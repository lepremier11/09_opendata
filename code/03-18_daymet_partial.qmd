...
install.packages("sf") # manipulate verctor geospatial coordinates
install.packages("daymetr") # API in R to access data base daymetr
install.packages("remotes") # CRAN : Appstore for R.
remotes::install_github("ropensci/USAboundaries") # use when we want to use a package 
...
```{r}
field <- read_csv("C:/Users/Bency Appolon/OneDrive - University of Georgia/41/UGA GO/UGA Courses/UGA Spring 25/CRSS 8030 - Stat/09_opendata/data/cotton_fielddata.csv")
...
```{r}
states <- us_states() %>%
  filter( !(state_abbr %in% c("PR", "AK", "HI"))) # ! exclamation negates the logic in the parentheses, %in% when we want to select more than one
...
daymet_all <- field %>%
  mutate(weather = pmap(list(.y = year,
                        ...
                        function(.y, .site, .lat, .lon)
                          ...
                          rename(.year = year,
                                 .site = site)## silent just quiets down the run
                             ) ) ### 
